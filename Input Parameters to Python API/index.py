...
class queryParamRequestHandler(tornado.web.RequestHandler):  #building a GET endpoint with a query string parameter
    def get(self):
        num = self.get_argument("num")
        if (num.isdigit()):
            r = "odd" if int(num) % 2 else "even"
            self.write(f"{num} is an integer and it's {r}")
        else:
            self.write(f"{num} is not an integer")

class resourceParamRequestHandler(tornado.web.RequestHandler): #building a GET endpoint with resource/path parameters
    def get(self, studentName, courseId):
        self.write(f"Welcome {studentName}! You are viewing the course id : {courseId}")
...
if __name__ == "__main__":
    app = tornado.web.Application([
        ...
        (r"/isEven",queryParamRequestHandler),  #ex: localhost:8882/isEven?num=9
        (r"/students/([a-z]+)/([0-9]+)",resourceParamRequestHandler)  #ex: localhost:8882/students/indhu/130502 #regex101.com
    ])
    port = 8882
    app.listen(port)
